...
def get_categories():
    categories = db.session.execute(
        "SELECT Category FROM Categories ORDER BY Category").fetchall()
    return result_to_array(categories)


def get_counties():
    counties = db.session.execute(
        "SELECT County FROM Counties ORDER BY County").fetchall()
...
def get_category_id(category_name):
    ...
    category_id = db.session.execute(
        sql, {"category_name": category_name}).fetchone()[0]
    return int(category_id)


def get_county_id(county_name):
    sql = "SELECT id FROM Counties WHERE County=:county_name"
    county_id = db.session.execute(
        sql, {"county_name": county_name}).fetchone()[0]
